...
#define OVSWIN_NW_FRAG_LATER (1 << 1) // Set for IP frag with nonzero offset.
...
typedef struct OvsFlowInfo {
    ...
    uint32_t actionsLen;
    struct nlattr actions[0];    // variable sized array
} OvsFlowInfo;
...
typedef struct OvsFlowDumpInput {
    uint32_t dpNo;
    uint32_t position[2];       // offsets to start dump from
                                // 0 - index within the same hash (ie. column)
                                // 1 - index in the flow hash table (ie. row)
...
typedef struct OvsFlowDumpOutput {
    uint32_t position[2]; // offsets to start the next dump operation.
    uint32_t n;           // #flows (currently 0 or 1). In case the buffer is
                          // too small to output all actions, this field
                          // indicates actual size needed to dump all actions.
    OvsFlowInfo flow;
} OvsFlowDumpOutput;
...
typedef struct OvsPacketExecute {
   ...
   uint32_t actionsLen;
   union {
       /* Variable sized data. actions succeeds packet data. */
       char packetBuf[0];
