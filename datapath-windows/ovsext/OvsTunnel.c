...
OvsTunnelAnalyzePacket(OVS_TUNNEL_PENDED_PACKET *packet)
{
    ...
    copiedNBL = OvsAllocateVariableSizeNBL(ovsSwitchContext, packetLength,
...
analyzeFreeNBL:
    OvsCompleteNBL(ovsSwitchContext, copiedNBL, TRUE);
analyzeDone:
                              ...
                              OVS_TUNNEL_PENDED_PACKET *packet)
{
...

    {
        ...
        NdisAcquireRWLockRead(ovsSwitchContext->dispatchLock, &lockState, dispatch);
        ...
        if (flow) {
            ...
            OvsActionsExecute(ovsSwitchContext, &completionList, pNbl,
...
unlockAndDrop:
    ...
    NdisReleaseRWLock(ovsSwitchContext->dispatchLock, &lockState);
dropit:
    pNbl = OvsCompleteNBL(ovsSwitchContext, pNbl, TRUE);
    ASSERT(pNbl == NULL);
    return status;
}
