...
OvsLookupIPNeighEntry(UINT32 ipAddr)
{
    ...
    UINT32 hash = jhash_words(&ipAddr, 1, OVS_HASH_BASIS);
...
OvsHashIPPrefix(PIP_ADDRESS_PREFIX prefix)
{
    ...
    return jhash_words((UINT32 *)&words, 2, OVS_HASH_BASIS);
}
...
OvsLookupIPFwdEntry(UINT32 dstIp)
{
    ...
    UINT32 hash = jhash_words(&dstIp, 1, OVS_HASH_BASIS);
...

{
    ...
    if (ipn->refCount == 0) {
        ...
        NdisReleaseSpinLock(&ovsIpHelperLock);
        hash = jhash_words(&ipn->ipAddr, 1, OVS_HASH_BASIS);
    ...
    hash = jhash_words(&fwdEntry->info.dstIpAddr, 1, OVS_HASH_BASIS);
