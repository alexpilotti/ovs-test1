                     ...
                     BOOLEAN resetTunnelInfo)
{
    ...
    if (fwdDetail->NumAvailableDestinations > 0) {
         ...
         */
        switchContext->sxSwitch->NdisSwitchHandlers.GetNetBufferListDestinations(
            switchContext->sxSwitch->NdisSwitchContext, curNbl,
            ...
            BOOLEAN preservePriority)
{
    ...
    NDIS_STATUS status;
    PSX_SWITCH_OBJECT sxSwitch = ovsFwdCtx->switchContext->sxSwitch;
    ...
    if (ovsFwdCtx->destPortsSizeOut == ovsFwdCtx->destPortsSizeIn) {
        if (ovsFwdCtx->destPortsSizeIn == 0) {
            ...
            status =
                sxSwitch->NdisSwitchHandlers.GrowNetBufferListDestinations(
                    sxSwitch->NdisSwitchContext, ovsFwdCtx->curNbl,
             ...
             */
            status = sxSwitch->NdisSwitchHandlers.UpdateNetBufferListDestinations(
                sxSwitch->NdisSwitchContext, ovsFwdCtx->curNbl,
                ovsFwdCtx->fwdDetail->NumAvailableDestinations,
                ovsFwdCtx->destinationPorts);
            if (status != NDIS_STATUS_SUCCESS) {
            ...
            status = sxSwitch->NdisSwitchHandlers.GrowNetBufferListDestinations(
                sxSwitch->NdisSwitchContext, ovsFwdCtx->curNbl,
 ...
 * --------------------------------------------------------------------------
 * OvsTunnelPortRx --
 *     The start function for Rx tunneling - decapsulates the packet, and
 *     outputs the packet on the INT bridge.
 *
 * Side effects:
 *     The NBL in 'ovsFwdCtx' is consumed.
 * --------------------------------------------------------------------------
...
OvsOutputForwardingCtx(OvsForwardingContext *ovsFwdCtx)
{
    ...
    if (ovsFwdCtx->destPortsSizeOut > 0) {
        ...
        SxLibSendNetBufferListsIngress(ovsFwdCtx->switchContext->sxSwitch,
                                       ovsFwdCtx->curNbl,
                                       ovsFwdCtx->sendFlags, 0);
 ...
 *     XXX: Assumes that the packet is owned by OVS. Will change with the buffer
 *     management adoption.
 * --------------------------------------------------------------------------
 */
static __inline NDIS_STATUS
OvsPopVlanInPktBuf(OvsForwardingContext *ovsFwdCtx)
{
    ...
    /*
     * XXX: Declare a dummy vlanTag structure since we need to compute the size
     * of the shiftLength. The NDIS one is a unionized structure.
     */
                   ...
                   const struct ovs_key_ethernet *ethAttr)
{
    ...
    ASSERT(curNb->Next == NULL);
    curMdl = NET_BUFFER_CURRENT_MDL(curNb);
    NdisQueryMdl(curMdl, &bufferStart, &len, LowPagePriority);
    if (!bufferStart) {
        ovsActionStats.noResource++;
        return NDIS_STATUS_RESOURCES;
    }
    len -= NET_BUFFER_CURRENT_MDL_OFFSET(curNb);
    ASSERT(len > 0);
                    ...
                    const struct nlattr *a)
{
    ...
    {
        OvsIPv4TunnelKey tunKey;
        if (type == OVS_KEY_ATTR_IPV4_TUNNEL) {
            OvsIPv4TunnelKey *tmp;
            tmp = (OvsIPv4TunnelKey*)nl_attr_data(a);
            tunKey.attr[0] = tmp->attr[0];
            tunKey.attr[1] = tmp->attr[1];
            tunKey.attr[2] = tmp->attr[2];
        } else {
            /* XXX: Will we ever have to convert from a key to nlattr? */
