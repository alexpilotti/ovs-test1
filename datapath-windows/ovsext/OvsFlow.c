                     ...
                     PSX_SWITCH_OBJECT sxSwitch)
{
    ...
    datapath->lock = NdisAllocateRWLock(sxSwitch->NdisFilterHandle);
...
OvsHashFlow(const OvsFlowKey *key)
{
    ...
    ASSERT(!key->tunKey.dst || offset == 0);
    start = (UINT8 *)key + offset;
    return jhash_bytes(start, size, 0);
}
               ...
               UINT32 *replyLen)
{
    ...
    if (status == STATUS_BUFFER_TOO_SMALL) {
        ...
        *replyLen = sizeof(OvsFlowDumpOutput) + dumpOutput->flow.actionsLen;
    }
               ...
               OvsFlowInfo *info)
{
    ...
    if ((getFlags & FLOW_GET_ACTIONS) && flow->actionsLen && getActionsLen) {
        if (flow->actionsLen > getActionsLen) {
            info->actionsLen = 0;
