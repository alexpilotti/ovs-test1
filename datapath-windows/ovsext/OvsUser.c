...
{
    return switchContext->sxSwitch->NdisSwitchHandlers.
        AllocateNetBufferListForwardingContext(
            switchContext->sxSwitch->NdisSwitchContext, nbl);
}
                  ...
                  UINT32 outputLength)
{
    ...
    sxSwitch = ovsSwitchContext->sxSwitch;
    ASSERT(sxSwitch);
     ...
     */
    pNbl = OvsAllocateNBLForUserBuffer(ovsSwitchContext, &execute->packetBuf,
                                       execute->packetLen);
    if (pNbl == NULL) {
    ...
    if (ndisStatus == NDIS_STATUS_SUCCESS) {
        ...
        ndisStatus = OvsActionsExecute(ovsSwitchContext, NULL, pNbl,
    ...
    if (pNbl) {
        OvsCompleteNBL(ovsSwitchContext, pNbl, TRUE);
    }
unlock:
...
#define VLAN_TAG_SIZE 4
    UINT32 allocLen, dataLen, offset, len, extraLen = 0;
    POVS_PACKET_QUEUE_ELEM elem;
    PMDL mdl;
    UINT8 *src, *dst;
    ...
    do {
        NdisQueryMdl(mdl, &src, &len, NormalPagePriority);
        if (src == NULL) {
            OvsFreeMemory(elem);
            ovsUserStats.dropDuetoResource++;
            return NULL;
                ...
                UINT32 numElems)
{
    ...
    OVS_LOG_LOUD("==> queueId %u, numELems: %u",
                  queueId, numElems);
    if (queue == NULL) {
        goto cleanup;
    }
...
cleanup:
    ...
    OVS_LOG_LOUD("<== drop %u packets", num);
}
