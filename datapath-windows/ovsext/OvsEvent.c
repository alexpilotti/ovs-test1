             ...
             UINT32 status)
{
    ...
    OVS_LOG_TRACE("<= portNo: %#x, status: %#x", portNo, status);
    ...
    OVS_LOG_TRACE("=> triggered pollAll: %s",
                       ...
                       UINT32 inputLength)
{
    ...
    OVS_LOG_TRACE("=> fileObject: %p, inputLength: %d", fileObject,
                  inputLength);

    if (inputLength < sizeof (OVS_EVENT_SUBSCRIBE) ||
        (request->mask & OVS_EVENT_MASK_ALL) == 0) {
        OVS_LOG_TRACE("<= subscribe failed with invalid request.");
...
done_event_subscribe:
    ...
    OVS_LOG_TRACE("<= subscribe event with status: %#x.", status);
    return status;
}
                  ...
                  UINT32 *replyLen)
{
    ...
    *replyLen = sizeof (OVS_EVENT_STATUS) + sizeof (OVS_EVENT_ENTRY);
    ...
    if (instance == NULL) {
        OvsReleaseEventQueueLock();
        *replyLen = 0;
        OVS_LOG_TRACE("<= can not find Open instance");
        return STATUS_INVALID_PARAMETER;
    }
...
event_poll_done:
    ...
    *replyLen = sizeof (OVS_EVENT_STATUS) +
    ...
    OVS_LOG_TRACE("<= numEventPolled: %d", numEntry);
    return STATUS_SUCCESS;
}
 ...
 *  Please note, when this routine is called, it is always
 *  guaranteed that IRP is valid.
 *
 * Results:
 *    None
 *
 * Side effects:
 *    pending IRP is completed.
 * --------------------------------------------------------------------------
 */
VOID
                  ...
                  UINT32 inputLength)
{
    ...
    OVS_LOG_TRACE("=> inputLength: %u", inputLength);

    if (inputLength < sizeof (OVS_EVENT_POLL)) {
        OVS_LOG_TRACE("<=Invalid input buffer length.");
        return STATUS_INVALID_PARAMETER;
    }
    poll = (POVS_EVENT_POLL)inputBuffer;
    ...
    if (instance == NULL) {
        OvsReleaseEventQueueLock();
        OVS_LOG_TRACE("<= Can not find open instance, dpNo: %d", poll->dpNo);
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (queue->pendingIrp) {
        OvsReleaseEventQueueLock();
        OVS_LOG_TRACE("<= Event queue already in pending state");
    ...
    OVS_LOG_TRACE("<= return status: %#x", status);
    return status;
}
