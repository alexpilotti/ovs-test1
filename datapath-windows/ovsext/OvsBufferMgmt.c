 ...
 *          SxLibCompleteNetBufferListsIngress()if the buffer is from upper
                  ...
                  NDIS_HANDLE adapterHandle)
{
    ...
    OVS_LOG_TRACE("==> context: %p, adapter: %p", context, adapterHandle);

    ovsPool = &context->ovsPool;
    RtlZeroMemory(ovsPool, sizeof (OVS_NBL_POOL));
    ovsPool->ndisHandle = adapterHandle;
    ovsPool->ndisContext = context->sxSwitch->NdisSwitchContext;
    /*
    ...
    ovsPool->fixSizePool = NdisAllocateNetBufferListPool(adapterHandle,
                                                         &nblParam);
    ...
    ovsPool->zeroSizePool = NdisAllocateNetBufferListPool(adapterHandle,
                                                          &nblParam);
    ...
    ovsPool->nblOnlyPool = NdisAllocateNetBufferListPool(adapterHandle,
                                                         &nblParam);
    ...
    nbParam.DataSize = 0;
    ovsPool->nbPool = NdisAllocateNetBufferPool(adapterHandle, &nbParam);
    if (ovsPool->nbPool == NULL) {
        goto pool_cleanup;
    }
    OVS_LOG_TRACE("<== fixSizePool: %p zeroSizePool: %p nblOnlyPool: %p"
...
pool_cleanup:
    ...
    OVS_LOG_TRACE("<=== Fail to initialize ovs buffer pool");
...
OvsCleanupBufferPool(PVOID ovsContext)
{
    ...
    POVS_SWITCH_CONTEXT context = (POVS_SWITCH_CONTEXT)ovsContext;
    ovsPool = &context->ovsPool;
    OVS_LOG_TRACE("==>  context: %p", context);
#ifdef DBG
    ...
    OVS_LOG_TRACE("<== cleanup OVS Buffer pool");
}
                      ...
                      UINT32 headRoom)
{
    ...
    status = context->sxSwitch->NdisSwitchHandlers.
                  ...
                  PNET_BUFFER_LIST nbl)
{
    ...
    context->sxSwitch->NdisSwitchHandlers.
 ...
 */
PNET_BUFFER_LIST
OvsCompleteNBL(PVOID ovsContext,
               PNET_BUFFER_LIST nbl,
               BOOLEAN updateRef)
{
    POVS_BUFFER_CONTEXT ctx;
    ...
    OVS_LOG_LOUD("==> nbl: %p, ctx: %p, refCount: %d, updateRef:%d",
